...
int main(int argc, char** argv)
{
  ...
  // STEP 2 - preselection
  step = 2;
  SetStepNames(stepNames, "preselection", step, stepNameVerbosity);
  ...
  for(int entry = entryFIRST ; entry < chain -> GetEntries() ; ++entry)
  {
    ...
    for(unsigned int jetIt = 0; jetIt < (reader.Get4V("jets")->size()); ++jetIt)
    {
      ...
      if(jetType == "PF")
      {
        ...
        if( (fabs(jet.eta()) < 2.4) && (reader.GetFloat("jets_chargedMuEnergyFraction")->at(jetIt) >= 0.99) ) continue;
