...
#tagJetDetaMIN loop
for($tagJetDetaMIN = 3; $tagJetDetaMIN < 6; $tagJetDetaMIN+=1)
{

#tagJetMJJMIN loop
for($tagJetMjjMIN = 500; $tagJetMjjMIN < 800; $tagJetMjjMIN+=100)
{
...
$OUTPUTSaveDirNew = $OUTPUTSaveDir."/tagJetDetaMIN_".$tagJetDetaMIN.
                                   "_tagJetMjjMIN_".$tagJetMjjMIN.
                                   "/";
$command = "mkdir ".$OUTPUTSaveDirNew;
print($command."\n");
system($command);

$command = "rm ".$OUTPUTSaveDirNew."/tree.root";
print($command."\n");
system($command);


$sampleJobListFile = $OUTPUTSaveDirNew."/lancia.sh";
...
while(<SAMPLESListFile>)
{
  ...
  $sampleDir = $OUTPUTSaveDirNew.$sample."/";
  ...
  system("cat ".$SELECTIONSCfgTemplate."   | sed -e s%INPUTFILELIST%".$inputFileList.
                                       ...
                                       "%g | sed -e s%TAGJETDETAMIN%".$tagJetDetaMIN.
                                       "%g | sed -e s%TAGJETMJJMIN%".$tagJetMjjMIN.
                                       "%g > ".$selectionsCfgFile);
