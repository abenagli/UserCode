...
void higgsMassFit(const std::string& mode = "exclusion", const bool& drawPlots = false)
{
  ...
  //-------------------
  // define the outfile
  TFile* outFile = new TFile("higgsMassFit_H400.root","RECREATE");
  outFile -> cd();
  ...
  std::string BKGPath = "/grid_mnt/vol__vol1__u/llr/cms/abenagli/COLLISIONS7TeV/Fall10/VBFAnalysisPackage/data/VBFAnalysis_AK5PF_H400_ET30_maxDeta_minDeta_Spring11_EGMu_noHiggsMassCut/";
  ...
  std::string GluGluHToLNuQQFolder = "GluGluToHToWWToLNuQQ_M-400_7TeV-powheg-pythia6_Spring11-PU_S1_START311_V1G1-v1/";
  std::string GluGluHToTauNuQQFolder = "GluGluToHToWWToTauNuQQ_M-400_7TeV-powheg-pythia6_Spring11-PU_S1_START311_V1G1-v1/";
  std::string VBFHToLNuQQFolder = "VBF_HToWWToLNuQQ_M-400_7TeV-powheg-pythia6_Spring11-PU_S1_START311_V1G1-v1/";
  std::string VBFHToTauNuQQFolder = "VBF_HToWWToTauNuQQ_M-400_7TeV-powheg-pythia6_Spring11-PU_S1_START311_V1G1-v1/";
  ...
  if( drawPlots )
  {
    ...
    c1 -> Print("BKGShapeStack.png","png");
  }  
  ...
  if( drawPlots )
  {
    ...
    c2 -> Print("BKGShapeHistoNorm.png","png");
  }
  ...
  x.setRange("low",     0., 330.);
  x.setRange("signal",330., 480.);
  x.setRange("high",  480.,1000.);
  
  float NBKGToy_signal = rooBKGToyDataSet->sumEntries("x>330. && x<480.");
  ...
  for(int j = 0; j < 10000; ++j)
  {
    ...
    //NSIGToy = S.Poisson(SIGShapeHisto->Integral());
    NSIGToy = 0;
    ...
    //NBKGToy_signal = rooBKGToyDataSet->sumEntries("x>330. && x<480.");
    //NSIGToy_signal = rooSIGToyDataSet->sumEntries("x>330. && x<480.");
    
    
    //rooTotPdf -> fitTo(*rooBKGToyDataSet,Extended(kTRUE),PrintLevel(-1),Verbose(false),Range("low,high"));
    rooTotPdf -> fitTo(*rooBKGToyDataSet,Extended(kTRUE),PrintLevel(-1),Verbose(false));
    
    
    //NBKGToy_fit = 0;
    //for(int i = 0; i < nBKG; ++i)
    //{
    //  RooAbsReal* rooBKGIntegral = rooBKGPdf[i] -> createIntegral(x,NormSet(x),Range("signal"));
    //  NBKGToy_fit += rooBKGIntegral->getVal() * rooNBKGTot->getVal();
    //}
