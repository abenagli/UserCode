...
int main (int argc, char** argv)
{
	...
	if(final)
	{
		...
		if(subtractWW   == true) signalRegionWW = (TH1F *) m4_signal_WW->Clone ("signalRegionWW") ;
		if(subtractDY   == true) signalRegionDY = (TH1F *) m4_signal_DY->Clone ("signalRegionDY") ;
		if(subtractTT   == true) signalRegionTT = (TH1F *) m4_signal_TT->Clone ("signalRegionTT") ;
		if(subtracttop  == true) signalRegiontop = (TH1F *) m4_signal_top->Clone ("signalRegiontop") ;
		if(subtractWZ   == true) signalRegionWZ = (TH1F *) m4_signal_WZ->Clone ("signalRegionWZ") ;
		if(subtractWJets == true)signalRegionWJets = (TH1F *) m4_signal_WJets->Clone ("signalRegionWJets") ;
		if(subtractZZ   == true) signalRegionZZ = (TH1F *) m4_signal_ZZ->Clone ("signalRegionZZ") ;
	}
    ...
    TH1F * h_correctionBand = new TH1F("h_correctionBand","h_correctionBand",nBins,m4_min,m4_max);
    for (int iBin = 1 ; iBin <= h_correctionBand->GetNbinsX () ; ++iBin)
    {    
        double center = h_correctionBand->GetBinCenter (iBin) ;
        int binInRebinned = ratio_total->GetXaxis ()->FindBin (center) ;

        double content = ratio_total->GetBinContent (binInRebinned) ;
        double error = ratio_total->GetBinError (binInRebinned) ;
	...
	TFile output ("output_018_" + flavour + ".root", "recreate") ;
	output.cd () ;
