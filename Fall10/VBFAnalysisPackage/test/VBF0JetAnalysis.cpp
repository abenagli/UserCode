...
int main(int argc, char** argv)
{
  ...
  if(argc != 2)
  {
    std::cerr << ">>>>> VBF0JetAnalysis::usage: " << argv[0] << " configFileName" << std::endl ;
    return 1;
  }
  ...
  std::cout << ">>>>> VBF0JetAnalysis::inputFileList = " << inputFileList << std::endl;
  ...
  std::cout << ">>>>> VBF0JetAnalysis::Read " << chain -> GetEntries() << " entries" << std::endl;  
  TRandom3 r;
  for(int entry = entryFIRST ; entry < chain -> GetEntries() ; ++entry)
  {
    ...
    if((entry%entryMODULO) == 0) std::cout << ">>>>> VBF0JetAnalysis::GetEntry " << entry << std::endl;   
    ...
    //**********************************
    // STEP 15 - Initial cuts - jet veto
    step += 1;
    SetStepNames(stepNames, "jet veto", step, verbosity);
    
    
    if( (vars.nJets_et20) > 2 ) continue;    
