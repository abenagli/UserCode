...
int main(int argc, char** argv)
{
  ...
  //HLTPathNames_mu_DATA.push_back("any");
  ...
  stepNames[6]  = "7) Lepton mt";
  stepNames[7]  = "8) Met";
  stepNames[8]  = "9) 2! W-jets";
  stepNames[9]  = "10) b-tag veto";
  stepNames[10] = "11) lep-met angle cuts";
  stepNames[11] = "12) WJ1-WJ2 angle cuts";
  stepNames[12] = "13) Pt max cuts";
  stepNames[13] = "14) Helicity angles cuts";
  stepNames[14] = "15) W mass cut";
  stepNames[15] = "16) Higgs mass cut";
  ...
  for(int entry = entryFIRST ; entry < chain -> GetEntries() ; ++entry)
  {
    ...
    {
      if( vars.lep_tkIso/vars.lep.pt()  > 0.125 ) continue;
      if( vars.lep_emIso/vars.lep.pt()  > 0.125 ) continue;
      if( vars.lep_hadIso/vars.lep.pt() > 0.125 ) continue;
    ...
    {
      if( vars.lep_tkIso/vars.lep.pt()  > 0.075 ) continue;
      if( vars.lep_emIso/vars.lep.pt()  > 0.075 ) continue;
      if( vars.lep_hadIso/vars.lep.pt() > 0.075 ) continue;
    ...
    if( (vars.lep_flavour == 11) && (vars.met.Et() < 30.) ) continue;
    ...
    if( (vars.lep_flavour == 13) && (vars.lep.pt() < 27.) ) continue;
    if( (vars.lep_flavour == 13) && (fabs(vars.lep.eta()) > 2.1) ) continue;
    if( vars.lep_tkIso > 1.000 ) continue;    
    ...
    //************************************
    // STEP 15 - Initial cuts - W mass cut
    step += 1;
    ...
    //****************************************
    // STEP 16 - Initial cuts - Higgs mass cut
    step += 1;
