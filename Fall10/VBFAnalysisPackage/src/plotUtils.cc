...
void drawTStack::Draw(TCanvas* c, const std::string& histoName, const std::string& mode, const bool& stackSig, const bool& logy, const float& lumi)
{
  ...
  if( (mode == "eventsScaled") || (mode == "sameAreaStack") )
  {
    ...
    ratioHisto -> GetYaxis() -> SetRangeUser(0.6, 1.4);
  ...
  if(m_drawLegend == true)
  {
    ...
    char lumiBuffer[200];
    if( mode == "eventsScaled" )
      sprintf(lumiBuffer, "CMS 2011        #sqrt{#font[52]{s}} = 7 TeV        #scale[0.75]{#int}#font[52]{L}d#font[52]{t} = %.1f fb^{-1}",lumi);
    if( mode == "sameAreaNoStack" )
      sprintf(lumiBuffer, "CMS 2011        histograms normalized to unit area");
    if( mode == "sameAreaStack" )
      sprintf(lumiBuffer, "CMS 2011        #sqrt{#font[52]{s}} = 7 TeV        #scale[0.75]{#int}#font[52]{L}d#font[52]{t} = %.1f fb^{-1}",lumi);
    if( mode == "integralStack" )
      sprintf(lumiBuffer, "CMS 2011        integral distributions");
    if( mode == "integralNoStack" )
      sprintf(lumiBuffer, "CMS 2011        integral distributions");
    
    latex = new TLatex(0.13, 0.94, lumiBuffer);
