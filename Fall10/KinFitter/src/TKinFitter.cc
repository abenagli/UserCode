...
void TKinFitter::addMeasParticle( TAbsFitParticle* particle ) {
  ...
  if ( particle != 0 ) {
    ...
    std::cerr << "NullPointer " << "Measured particle points to NULL. It will not be added to the KinFitter.";
  }
...
void TKinFitter::addUnmeasParticle( TAbsFitParticle* particle ) {
  ...
  if ( particle != 0 ) {
    ...
    std::cerr << "NullPointer " << "Unmeasured particle points to NULL. It will not be added to the KinFitter.";
  }
...
void TKinFitter::printMatrix(const TMatrixD &matrix, const TString name) {
  ...
  if (!matrix.IsValid()) {
    std::cerr << "InvalidMatrix " << "Matrix " << name << " is invalid.";
    return;
  }
